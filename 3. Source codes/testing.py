...
img_path='./TestData/data/'
results_path = './result_data/' # the path of enhanced images


def _parse_function(filename):   
  image_string = tf.read_file(filename)  
  image_decoded = tf.image.decode_png(image_string, channels=3)  
  underwater_image = tf.cast(image_decoded, tf.float32)/255.0   
...
if __name__ == '__main__':
   ...
   pyramid = model.inference(input_image)
   ...
   with tf.Session(config=config) as sess:
      with tf.device('/gpu:0'): 
          ...
          for i in range(num_img):     
             enhanced, ori = sess.run([output, input_image])              
